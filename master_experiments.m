...
P.ref_freq   = 1.0;       % Frequency for Sine Reference 
P.use_mrac   = 0;         
% --- Disturbance ---
...
fprintf('  11. Control Signal Frequency Analysis (Fixed Dist, Varying Filter BW) [Hz]\n');
...
fprintf('  15. State-Dependent Disturbance: BW Sweep & Signal Frequency Breakdown\n');
% fprintf('  16. LQR Stress Test: High Amp/Resonant Disturbance (BW Sweep)\n');
% fprintf('  17. Time Delay Margin vs Sampling Time Ts (Log Scale)\n');
% fprintf('  18. Time Delay Margin vs Filter Cutoff Frequency\n');
fprintf('  19. MRAC: Time Delay Margin vs Adaptation Gain (Gamma)\n');
fprintf('--------------------------------------------------\n');
...
switch choice
    ...
    case 2 % --- EXP 2: TRACKING PERFORMANCE (3-Way x 3 Refs) ---
        ...
        subtitle(tl, sprintf('Disturbance (Roll): $\\tau_\\phi = %.1f \\sin(%.1f t)$ Nm | (Applied to all cases)', P.dist_amp(1), P.dist_freq(1)), 'Interpreter','latex', 'FontSize', 12);
    ...
    case 8 % --- Filter Bandwidth vs Disturbance Frequency (2nd Order) ---
        ...
        fprintf('Doesnt work yet, need to debug: pending');
    ...
    case 11 % --- Control Signal Analysis (Fixed Dist, Varying Filter BW) ---
    ...
    case 17 % --- Time Delay Margin vs Ts ---
        fprintf('Running Time Delay Margin vs Ts Sweep...\n');
        Ts_vals = 100*[1e-4, 2e-4, 5e-4, 1e-3, 2e-3, 5e-3, 1e-2];
        tdm_results = [];
        rmse_at_tdm = []; 
        ...
        P.use_mrac = 0; P.Clpf = eye(3);
        P.dist_amp  = [1.0; 0.5; 0.2]; P.dist_freq = [2.0; 1.5; 0.5]; P.dist_type = 0;  
        ...
        for i = 1:length(Ts_vals)
            ...
            % Search for TDM: Fine resolution 
            % Sweep from 0 to 200ms in 1ms steps 
            delay_steps = 0:0.001:0.500; 
            
            stable_delay = 0;
            last_rmse = 0;
            
            for d_idx = 1:length(delay_steps)
                delay = delay_steps(d_idx);
                fprintf("search delay %.7f", delay)
                fprintf('--------------------------------------------------\n');
                
                P.loop_delay = delay;
                if P.loop_delay == 0, P.loop_delay = 1e-7; end % Avoid Simulink 0-delay issues
                
                assignin('base', 'P', P);
                
                try
                    % Fixed-step is often safer for stability it seems
                    % Using variable step, but with constraints
                    % out = sim(model_name, 'StopTime', '5.0'); 
                    % out = sim(model_name, 'StopTime', '5.0', ...
                    %           'Solver', 'ode45', ...
                    %           'RelTol', '1e-1', 'AbsTol', '1e-1', ...
                    %           'MinStep', '1e-6');
                    out = sim(model_name, 'StopTime', '20.0', ...
                              'Solver', 'ode23t', ...  
                              'RelTol', '1e-2', 'AbsTol', '1e-3', 'MinStep', '1e-6');
                    % out = sim(model_name, 'StopTime', '5.0', ...
                    %           'Solver', 'ode15s', ...  
                    %           'RelTol', '1e-3', 'AbsTol', '1e-4', 'MinStep', '1e-6');
                    % 
                    x_hist = out.sim_x.Data(:,1);
                    if size(x_hist, 2) > 1, x_hist = x_hist.'; end
                    error_val = x_hist - P.ref_amp;
                    
                    if max(abs(error_val)) > 100 
                         % Unstable!
                         fprintf(' [Fail at %.3f]', delay);
                         break; % Stop searching for this Ts
                    else
                         % Stable
                         stable_delay = delay; 
                         last_rmse = rad2deg(sqrt(mean(error_val.^2)));
                    end
                catch
                    % Simulation crashed (singularity)
                    % fprintf(' [Crash at %.3f]', delay);
                    break; 
                end
            end
            
            tdm_results(i) = stable_delay;
            rmse_at_tdm(i) = last_rmse;
            fprintf('TDM = %.4f s (RMSE: %.4f deg)\n', stable_delay, last_rmse);
        end
        
        plot(Ts_vals, tdm_results, '-o', 'LineWidth', 2, 'Color', 'b', 'MarkerFaceColor', 'b');

    case 18 % --- Time Delay Margin vs Filter Cutoff ---
        fprintf('Running Time Delay Margin vs Filter Cutoff Frequency Sweep...\n');
        
        % Parameters
        wc_vals = [5, 10, 20, 30, 40, 50, 75, 100]; % rad/s
        tdm_results = [];
        ...
        figure('Color','w','Position',[100 100 800 600]); hold on; grid on;
        title('Time Delay Margin vs Filter Bandwidth', 'Interpreter','latex');
        xlabel('Filter Cutoff $\omega_c$ (rad/s)', 'Interpreter','latex'); 
        ylabel('Time Delay Margin (s)', 'Interpreter','latex');
        set(gca, 'TickLabelInterpreter','latex', 'FontSize',12);
        ...
        for i = 1:length(wc_vals)
            ...
            fprintf('Testing wc = %d rad/s... ', w_c);
            
            % Search for TDM
            delay_steps = 0.001:0.0005:0.100; 
            stable_delay = 0;
            
            for d_idx = 1:length(delay_steps)
                delay = delay_steps(d_idx);
                fprintf("search delay %.7f \n", delay)
                % fprintf('--------------------------------------------------\n');
                ...
                try
                    % out = sim(model_name, 'StopTime', '2.0'); %, ...
                    %           % 'Solver', 'ode45', ...
                    %           % 'RelTol', '1e-2', 'AbsTol', '1e-3');
                    out = sim(model_name, 'StopTime', '5.0', ...
                              'Solver', 'ode45', ...  
                              'RelTol', '1', 'AbsTol', '1', 'MinStep', '1e-6');
                    
                    x_hist = out.sim_x.Data(:,1);
                    if size(x_hist, 2) > 1, x_hist = x_hist.'; end
                    error_val = x_hist - P.ref_amp;
                    
                    if max(abs(error_val)) > 10 
                         break; 
                    else
                         stable_delay = delay; 
                    end
        ...
        plot(wc_vals, tdm_results, '-s', 'LineWidth', 2, 'Color', [0.85 0.33 0.10], 'MarkerFaceColor', [0.85 0.33 0.10]);

    case 19 % --- MRAC: Time Delay Margin vs Gain ---
        fprintf('Running MRAC: Time Delay Margin vs Adaptation Gain (Gamma)...\n');
        
        % Parameters: Log-spaced Gammas
        gam_vals = [0.1, 0.5, 1, 5, 10, 20, 50, 100, 200, 500];
        tdm_results = [];
        
        % Setup: MRAC ON, L1 OFF
        P.Ts = 0.002; 
        P.Ae = -10.0 * eye(6);
        P.ref_type = 0; P.ref_amp = deg2rad(20); P.ref_start = 0; 
        P.use_mrac = 1; % MRAC Enabled
        P.Clpf = zeros(3); % L1 Disabled
        % P.dist_amp = [0;0;0]; % No Disturbance for Stability Margin
        P.dist_amp  = [1.0; 0.5; 0.2]; P.dist_freq = [2.0; 1.5; 0.5]; P.dist_type = 0;   
        
        
        % Relax bounds for high gain stability checks
        P.MRAC.theta_x_max = 500.0; P.MRAC.theta_r_max = 500.0; P.MRAC.theta_w_max = 500.0;
        
        figure('Color','w','Position',[100 100 800 600]); hold on; grid on;
        title('MRAC: Time Delay Margin vs Adaptation Gain', 'Interpreter','latex');
        xlabel('Adaptation Gain $\Gamma$ (Log Scale)', 'Interpreter','latex'); 
        ylabel('Time Delay Margin (ms)', 'Interpreter','latex');
        set(gca, 'XScale', 'log', 'TickLabelInterpreter','latex', 'FontSize',12);
        ...
        for i = 1:length(gam_vals)
            ...
            fprintf('Testing Gamma = %.1f... ', g);
            
            % Search for TDM
            % MRAC with high gain is very sensitive, search fine grid
            delay_steps = 0:0.001:0.100; 
            stable_delay = 0;
            ...
            for d_idx = 1:length(delay_steps)
                ...
                try
                    ...
                    out = sim(model_name, 'StopTime', '4.0', ...
                              'Solver', 'ode23t', ...
                              'RelTol', '1e-2', 'AbsTol', '1e-3', 'MinStep', '1e-6');
        ...
        plot(gam_vals, tdm_results * 1000, '-o', 'LineWidth', 2, 'Color', [0 0.45 0.74], 'MarkerFaceColor', [0 0.45 0.74]);
        % plot(gam_vals, tdm_results, '-o', 'LineWidth', 2, 'Color', [0 0.45 0.74], 'MarkerFaceColor', [0 0.45 0.74]);
end
%% ================= 3. HELPER FUNCTIONS =================
